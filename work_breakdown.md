...
| 1-3 | `CustomDataset` の最小実装（単一サンプルを読み込んで tensor を返す） | ★★☆ |
| 1-4 | 最小モデルの実装（まずは小さな 3D CNN / 既存 encoder の簡易版） | ★★☆ |
| 1-5 | 最小 loss と最小 train loop の実装（単一GPU, 単一 fold, AMP なしでも可） | ★★☆ |
| 1-6 | 学習ログの出力（loss, 学習率, 1 epoch の確認） | ★☆☆ |
